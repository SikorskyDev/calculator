(function () {
    ...
    list_city_1.on("click", "li", function () {
        ...
        const selectedOption = list_city_1.find(
            `li:contains(${selectedCityStart})`
        );
    ...
    list_street_1.on("click", "li", function () {
        ...
        // Зберегти значення Ref в змінну
        const selectedOption = list_street_1.find(
            `li:contains(${selectedStreetStart})`
        );
        if (selectedOption.length) {
    ...
    const craeteFirstCityList = () => {
        ...
        if (allCity_1.length) {
            $.each(allCity_1, function (index, name) {
                // для кожного елемента в масиві allCity_1
                const li = $("<li>", {
                    // створити елемент li
                    text: name.Present,
                    "data-id": name.Ref,
    ...
    const craeteFirstStreetList = () => {
        ...
        if (allStreet_1.length) {
            $.each(allStreet_1, function (index, stret) {
                // для кожного елемента в масиві allStreet_1
                const li = $("<li>", {
                    // створити елемент li
                    text: stret.Present,
                    class: "option_list-item",
    ...
    const getFinalyFirstCity = (event) => {
        const selectedOption = list_city_1.find(
            `li:contains(${event.target.value})`
        );
        if (selectedOption.length) {
            cityRef_1 = selectedOption.data("id");
        }
    ...
    const searchFirstCityFunc = async (inputCity) => {
        const obj = {
            apiKey: "a0d8dfcc826c892529232839d5c08747",
            modelName: "Address",
    ...
    list_city_2.on("click", "li", function () {
        ...
        // Зберегти значення Ref в змінну
        const selectedOption = list_city_2.find(
            `li:contains(${selectedCityStart})`
        );
        if (selectedOption.length) {
    ...
    list_street_2.on("click", "li", function () {
        ...
        // Зберегти значення Ref в змінну
        const selectedOption = list_street_2.find(
            `li:contains(${selectedStreetStart})`
        );
        if (selectedOption.length) {
    ...
    const craeteSecondCityList = () => {
        ...
        if (allCity_2.length) {
            $.each(allCity_2, function (index, name) {
                // для кожного елемента в масиві allCity_2
                const li = $("<li>", {
                    // створити елемент li
                    text: name.Present,
                    "data-id": name.Ref,
    ...
    const craeteSecondStreetList = () => {
        ...
        if (allStreet_2.length) {
            $.each(allStreet_2, function (index, stret) {
                // для кожного елемента в масиві allStreet_2
                const li = $("<li>", {
                    // створити елемент li
                    text: stret.Present,
                    class: "option_list-item",
    ...
    list_city_3.on("click", "li", function () {
        ...
        // Зберегти значення Ref в змінну
        const selectedOption = list_city_3.find(
            `li:contains(${selectedCityStart})`
        );
        if (selectedOption.length) {
    ...
    list_street_3.on("click", "li", function () {
        ...
        // Зберегти значення Ref в змінну
        const selectedOption = list_street_3.find(
            `li:contains(${selectedStreetStart})`
        );
        if (selectedOption.length) {
    ...
    const craeteThirdCityList = () => {
        ...
        if (allCity_3.length) {
            $.each(allCity_3, function (index, name) {
                // для кожного елемента в масиві allCity_3
                const li = $("<li>", {
                    // створити елемент li
                    text: name.Present,
                    "data-id": name.Ref,
    ...
    const craeteThirdStreetList = () => {
        ...
        if (allStreet_3.length) {
            $.each(allStreet_3, function (index, stret) {
                // для кожного елемента в масиві allStreet_3
                const li = $("<li>", {
                    // створити елемент li
                    text: stret.Present,
                    class: "option_list-item",
    ...
    list_city_4.on("click", "li", function () {
        ...
        // Зберегти значення Ref в змінну
        const selectedOption = list_city_4.find(
            `li:contains(${selectedCityStart})`
        );
        if (selectedOption.length) {
    ...
    list_street_4.on("click", "li", function () {
        ...
        // Зберегти значення Ref в змінну
        const selectedOption = list_street_4.find(
            `li:contains(${selectedStreetStart})`
        );
        if (selectedOption.length) {
    ...
    const craeteQuarterCityList = () => {
        ...
        if (allCity_4.length) {
            $.each(allCity_4, function (index, name) {
                // для кожного елемента в масиві allCity_4
                const li = $("<li>", {
                    // створити елемент li
                    text: name.Present,
                    "data-id": name.Ref,
    ...
    const craeteQuarterStreetList = () => {
        ...
        if (allStreet_4.length) {
            $.each(allStreet_4, function (index, stret) {
                // для кожного елемента в масиві allStreet_4
                const li = $("<li>", {
                    // створити елемент li
                    text: stret.Present,
                    class: "option_list-item",
    ...
    list_city_5.on("click", "li", function () {
        ...
        // Зберегти значення Ref в змінну
        const selectedOption = list_city_5.find(
            `li:contains(${selectedCityStart})`
        );
        if (selectedOption.length) {
    ...
    list_street_5.on("click", "li", function () {
        ...
        // Зберегти значення Ref в змінну
        const selectedOption = list_street_5.find(
            `li:contains(${selectedStreetStart})`
        );
        if (selectedOption.length) {
    ...
    const craeteFifthCityList = () => {
        ...
        if (allCity_5.length) {
            $.each(allCity_5, function (index, name) {
                // для кожного елемента в масиві allCity_5
                const li = $("<li>", {
                    // створити елемент li
                    text: name.Present,
                    "data-id": name.Ref,
    ...
    const craeteFifthStreetList = () => {
        ...
        if (allStreet_5.length) {
            $.each(allStreet_5, function (index, stret) {
                // для кожного елемента в масиві allStreet_5
                const li = $("<li>", {
                    // створити елемент li
                    text: stret.Present,
                    class: "option_list-item",
    ...
    $(".cars-list-item").on("click", function (e) {
        ...
        $(".dropdown-cars").html($(".car-selected ").parent().html());
        $(".cars-list").toggleClass("cars-list-visible");
    });

    $(document).on("click", function (event) {
        if (
            !$(event.target).closest(".calc-cars").length &&
            !$(event.target).is(".calc-cars")
        ) {
    ...
    // Додаю по кліку на кнопку #submitPhoneBtn клас redInputBorder до пустих інпутів та видаляю клас hidden із div який виводить текст із помилкою
    ...
    // Видаляю клас redInputBorder із інпутів які були закриті по натисканню на кнопку .close__adress
    $(".close__adress").click(function () {
        $(this).closest(".address").find("input").removeClass("redInputBorder");
        // Перевіряємо, чи залишилися пусті інпути з класом "redInputBorder"
        if ($inputs.filter(".redInputBorder").length === 0) {
            // Якщо не залишилося, то додаємо клас "hidden"
            $(".form-sent-error-emptyInputs").addClass("hidden");
        }
    });

    // Видаляю клас redInputBorder із інпутів на який натиснув корситувач та, якщо все ОК, то видаляю текст із помилкою.
    $inputs.focus(function () {
        $(this).removeClass("redInputBorder");

        // Перевіряємо, чи залишилися пусті інпути з класом "redInputBorder" та чи вибрані грузчики, якщо все ОК, то видаляю текст із помилкою
        if (
            $inputs.filter(".redInputBorder").length === 0 &&
    ...
    // Додаю redInputBorder та повідомлення із помилкою, якщо не вибрано кількість грузчиків
    $("#submitPhoneBtn").click(function () {
    ...
    // Видаляю клас redInputBorder із блоку із  грузчиками та, якщо все ОК, то видаляю текст із помилкою.
    $(".label__mover").on("click", function () {
        if ($(".label__mover-active").length !== 0) {
            $(".count").removeClass("redInputBorder");
        }
    ...
    $("#submitPhoneBtn").on("click", function () {
        ...
        if (inputValue.indexOf("_") !== -1 || inputValue.length < 19) {
            ...
            $("#calc-phone").addClass("redInputBorder");
    ...
    //Видаляю redInputBorder коли користувач клікає на інпут із номером та видаляю повідомлення із помилкою
    $("#calc-phone").focus(function () {
        $("#calc-phone").removeClass("redInputBorder");
        $(".form-sent-error").addClass("hidden");
    });
    ...
    $(".send").on("click", function () {
        ...
        if (formInputCurrent !== false) {
            ...
            // ====== Кнопка Відправки======//
            // для рус
            $(".submitPhoneBtn-calc").on("click", function () {
                $(this)
                    .addClass("green-ru")
                    .prop("disabled", true)
                    .text("Отправлено");
            ...
            // для укр
            // $('.submitPhoneBtn-calc').on('click', function() {
            // 	$(this).addClass('green-ua').prop('disabled', true).text('Відправлено');
            // });
        } else {
